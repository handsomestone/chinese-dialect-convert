 ...
 * @version 1.0
 */
public class SiChuanSegmenter implements ISegmenter {
	
	public static String LanType = "SiChuan";
	/*
	 * 已完成处理的位置
	 */
	...
	public void nextLexeme(char[] segmentBuff , Context context) {
		...
		if(CharacterHelper.isCJKCharacter(input)){//是（CJK）字符，则进行处理
			...
			//处理以input为开始的一个新hit
			Hit hit = LanDictionary.matchInSichuanDict(segmentBuff, context.getCursor() , 1);
			if(hit.isMatch()){//匹配成词
				//判断是否有不可识别的词段
